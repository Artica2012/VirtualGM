...
class PF2NpcSelectButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        async with session.begin():
            session.add(
                Condition(
                    character_id=character.id, title="AC", number=self.data.ac + stat_mod, counter=True, visible=False
                )
            )
            session.add(
                Condition(
            ...
            session.add(
                Condition(
                    character_id=character.id, title="DC", number=self.data.dc + stat_mod, counter=True, visible=False
                )
            )
            await session.commit()
        ...
        async with session.begin():
            if guild.initiative is not None:
                if not await initiative.init_integrity_check(
                    self.ctx, guild.initiative, guild.saved_order, self.engine
                ):
