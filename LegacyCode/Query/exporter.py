...
from Backend.Database.database_operations import get_db_engine
...
def disease_export(data):
    ...
    if not inspect(engine).has_table(emp):
        for row in data:
            try:
                stmt = emp.insert().values(Type=row["Type"], ID=row["ID"], Title=row["Title"], URL=row["URL"])
                stmt.compile()
        ...
        for row in data:
            try:
                stmt = emp.insert().values(
                    Type=row["Type"], ID=row["ID"], Title=row["Title"], Category=row["Category"], URL=row["URL"]
                )
                stmt.compile()
                with engine.connect() as conn:
                    conn.execute(stmt)
