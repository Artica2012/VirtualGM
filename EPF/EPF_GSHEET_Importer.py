...
async def epf_g_sheet_import(ctx: discord.ApplicationContext, char_name: str, base_url: str, engine=None, guild=None):
    try:
        ...
        if df.columns[1] == "Info:":
            return await epf_g_sheet_character_import(ctx, char_name, df, engine, guild)
        elif df.columns[1] == "Eidolon":
            return await epf_g_sheet_eidolon_import(ctx, char_name, df, engine, guild)
    except Exception:
...
async def epf_g_sheet_eidolon_import(ctx: discord.ApplicationContext, char_name:str, df, engine, guild):
    ...
    ##TODO Need to querty the partner's characer model here
