...
class HelpCog(commands.Cog):
    ...
    async def initiative(self, ctx: discord.ApplicationContext, command: str):
        if command == "char add":
            await ctx.respond(
                (
                    "```add - Add a PC or NPC\n- takes the argument player with the choice of player or NPC. NPCs have"
                    " their health - obscured and do not show custom counters on the non-gm tracker. - An argument of"
                    " _initiative_ can be given to set an initiative string (XdY+Z) which will be rolled when"
                    " initiative is started```"
                ),
                ephemeral=True,
            )
        elif command == "char edit":
            await ctx.respond(
                "``` Edit a PC or NPC- Edit the max hp and/or initiative string of a character you control  ```"
            )
        elif command == "char copy":
            await ctx.respond(
                "```Copies a character or NPC including any system specific stats. Does not copy and "
                "conditions or counters. Will roll a new initiative if initiative is active.```"
            )
        elif command == "char delete":
            await ctx.respond("``` Delete an  NPC- Deletes an NPC. Locked to the GM and the owner of the NPC  ```")
        elif command == "char sheet":
            await ctx.respond(
                "``` Show the character sheet- Shows a simplified character sheet for the character, including any"
                " system specific stats, conditions and counters  ```"
            )
        elif command == "manage":
            await ctx.respond(
                (
            ...
            await ctx.respond("```next - Advance Initiative (or start it)```", ephemeral=True)
        elif command == "init":
            await ctx.respond(
                (
                    "```"
    ...
    @option("command", choices=["m", "create", "show", "remove", "remove_all", "bulk_create"])
    async def macro(self, ctx: discord.ApplicationContext, command: str):
        if command == "m":
            await ctx.respond(
                (
                    "```Roll Macro\n- Select the character and the macro and VirtualGM will roll it for you.The options"
                    " secret argument will send the roll to the GM instead.\n- The optional dc argument will give a"
                    " positive (thumbsup) or negative (thumbsdown)  to indicate if the roll meets or exceeds the given"
                    " difficulty class.``` "
                ),
                ephemeral=True,
            )
            ...
            await ctx.respond(
                "```Remove Macro\n- Select the character and the macro, and this will delete it```", ephemeral=True
            )
        elif command == "show":
            await ctx.respond(
                (
                    "```Show Macros\n- Will display all macros for a character (including any over the 25 item limit in"
                    " the autocomplete box). You can roll macros by selecting them from the list.```"
                ),
                ephemeral=True,
            )
            ...
            await ctx.respond(
                (
                    "```Allows adding multiple macros at one time to the same character\n"
    ...
    @option("command", choices=["advance", "set"])
    async def timekeeping(self, ctx: discord.ApplicationContext, command: str):
        if command == "advance":
            await ctx.respond("```advance\n- advances the time by the selected amount``` ", ephemeral=True)
        elif command == "set":
            await ctx.respond(
                (
                    "```Set time\n"
    ...
    @option("command", choices=["attack", "save"])
    async def a(self, ctx: discord.ApplicationContext, command: str):
        if command == "attack":
            await ctx.respond("```attack\n- rolls an automatic attack``` ", ephemeral=True)
        elif command == "save":
