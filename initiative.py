    ...
    try:
        ...
        while not block_done:
            ...
            try:
                ...
                # con = await get_condition_table(ctx, metadata, engine)
                async with async_session() as session:
                    char_result = await session.execute(select(Condition).where(Condition.character_id == cur_char.id))
                ...
                for con_row in con_list:
                    ...
                    async with async_session() as session:
                        ...
                        if (
                            selected_condition.auto_increment and not selected_condition.time
                        ):  # If auto-increment and NOT time
    ...
    try:
        ...
        async with async_session() as session:
            char_result = await session.execute(
                ...
                .where(Condition.auto_increment is True)
            )
            con_list = char_result.scalars().all()
            logging.info("BAI9: condition's retrieved")
    ...
    async with async_session() as session:
        ...
        if gm_tracker is not None:
            gm_tracker_display_string = await block_get_tracker(
                await get_init_list(ctx, engine, guild=guild), guild.initiative, ctx, engine, bot, gm=True, guild=guild
            )
...
async def get_cc(ctx: discord.ApplicationContext, engine, bot, character: str):
    ...
    try:
        async with async_session() as session:
            result = await session.execute(
                select(Condition).where(Condition.character_id == character.id).where(Condition.counter is True)
            )
...
class InitiativeCog(commands.Cog):
    ...
    async def edit_char(self, ctx: discord.ApplicationContext, mode: str, character: str, condition: str):
        engine = get_asyncio_db_engine(user=USERNAME, password=PASSWORD, host=HOSTNAME, port=PORT, db=SERVER_DATA)
        result = False
        await ctx.response.defer(ephemeral=True)
        if mode == "delete":
